...
fn every_wall_in_corpus_generates_a_non_empty_mesh() {
    ...
    for (i, wall) in doc.walls.iter().enumerate() {
        ...
        assert_eq!(g.mesh_3d.vertex_count(), 24, "wall {i}: solid wall = 24 verts");
        assert_eq!(g.mesh_3d.triangle_count(), 12, "wall {i}: solid wall = 12 tris");
        assert!(g.cutouts.is_empty(), "wall {i}: no doors/windows in fixture");
...
fn every_floor_in_corpus_generates_a_non_empty_mesh() {
    ...
    for (i, floor) in doc.floors.iter().enumerate() {
        ...
        assert_eq!(g.mesh_3d.vertex_count(), 24, "floor {i}: 6 quads = 24 verts");
        assert_eq!(g.mesh_3d.triangle_count(), 12, "floor {i}: 6 quads = 12 tris");
...
fn corpus_walls_form_a_closed_perimeter_on_each_level() {
    ...
    let ground_floor_walls: Vec<_> = doc
        .walls
        .iter()
        .filter(|w| w.start.y == 0.0)
        .collect();
...
fn wall_plan_polygons_have_correct_perp_offset_for_axis_aligned_walls() {
    ...
    xs.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap().then(a.1.partial_cmp(&b.1).unwrap()));
